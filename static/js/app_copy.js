...
case_data.then(function(data) {
  ...
  function hospBar(outcome, hosp_arr, month_arr, year_arr, unique_months_arr) {
    ...
    let hosp_bar_data = [barTrace(Object.keys(hosp_yes_dict[2020]), Object.values(hosp_yes_dict[2020]), 2020), barTrace(Object.keys(hosp_yes_dict[2021]), Object.values(hosp_yes_dict[2021]), 2021) ]
    let bar_layout = {
    ...
    Plotly.newPlot("bar", hosp_bar_data, bar_layout);
  };
...
//----------------------------------------------------------------------------
    ...
    d3.select("#currSettings").on("change", updateHospBar);
