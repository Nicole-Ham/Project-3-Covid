...
case_data.then(function(data) {
  ...
  for(let i =0; i < types.length; i++) {
    ...
    selector_type.append("option").text(types[i]).property("value", types[i]);
  };
  ...
  function updateBar() {
    ...
    let type_dropdown = d3.select("#selType");
    let selected_type = type_dropdown.property("value");
    ...
    if (selected_county == "ALL COUNTIES") {
      ...
      plotBar(selected_outcome, curr_type_arr, curr_months_arr, curr_years_arr, unique_months);
